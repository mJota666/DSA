...
void flash_sort_comp(int arr[], int n, int& count_cmp) {
    count_cmp = 0;
    ...
    minValue = maxValue = arr[0];
    for (int i = 1;  i < n; i++) {
        if ( ++count_cmp && arr[i] < minValue)
            minValue = arr[i];
        else if ( ++count_cmp && arr[i] > maxValue)
            maxValue = arr[i];
    }
    count_cmp += n;
    // Initialize the L bucket with zeros
    ...
    int j = m - 1;
    int numMoves = 0; // Tracks the number of moves made during sorting
    while ( ++count_cmp && numMoves < n) {
        while ( ++count_cmp && i > L[j] - 1) {
            i++;
            j = static_cast<int>(static_cast<double>(m - 1) * (arr[i] - minValue) / (maxValue - minValue));
        }

        int flash = arr[i];
        while ( ++count_cmp && i != L[j]) {
            j = static_cast<int>(static_cast<double>(m - 1) * (flash - minValue) / (maxValue - minValue));
            int temp = arr[L[j] - 1];
            arr[L[j] - 1] = flash;
            flash = temp;
    ...
    // Perform insertion sort on each bucket
    for (int k = 1; ++count_cmp, k < m; k++) {
        for (int i = L[k] - 2; ++count_cmp, i >= L[k - 1]; i--) {
            int temp = arr[i];
            int j = i + 1;
            while ( ++count_cmp && temp > arr[j]) {
                arr[j - 1] = arr[j];
                j++;
            }
            arr[j - 1] = temp;
